...
void BtnGIODoneCallback(GUI_BUTTON *btn,INT32 reason)
{
	...
	if(reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		if(!is_networked)
		{
			//If there's no incompatibility problems
			if(DisplayMessageToUserAboutOIVandNASincompatibility()){
...
BOOLEAN DisplayMessageToUserAboutOIVandNASincompatibility()
{
	...
	//if the user has selected OIV mode with NAS
	if(!GetCurrentINVOptionButtonSetting() && gGameExternalOptions.fNewAttachmentSystem)
	{
		DoGioMessageBox( MSG_BOX_BASIC_STYLE, pMessageStrings[MSG_NAS_AND_OIV_INCOMPATIBLE], GAME_INIT_OPTIONS_SCREEN, MSG_BOX_FLAG_OK, NULL );
		return( FALSE );
	}
