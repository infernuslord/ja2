...
void ExitBriefingRoom()
{
	...
	for(i=0; i<BRIEFINGROOM_BUTTONS_DEF; i++)
		MSYS_RemoveRegion( &gSelectedBriefingRoomTocMenuRegion[i]);
	*/	
...
void RenderBriefingRoom()
{
	...
	/*
	GetVideoObject(&hContentButtonHandle, guiContentButtonBriefingRoom);

	usHeight = GetFontHeight(BRIEFINGROOM_FONT12ARIAL);
	usPosY = BRIEFINGROOM_CONTENTBUTTON_Y + 120;
	for(i=0; i<BRIEFINGROOM_BUTTONS_DEF; i++)
	{
	
	    idText = i + 16;
		BltVideoObject(FRAME_BUFFER, hContentButtonHandle, 0,BRIEFINGROOM_TOC_X, usPosY, VO_BLT_SRCTRANSPARENCY,NULL);

		usWidth = StringPixLength(pMenuStrings[idText], BRIEFINGROOM_FONT12ARIAL);
		DrawTextToScreen(pMenuStrings[idText], BRIEFINGROOM_TOC_X, (UINT16)(usPosY + BRIEFINGROOM_TOC_Y), BRIEFINGROOM_BUTTON_SIZE_X, BRIEFINGROOM_FONT12ARIAL, BRIEFINGROOM_FONT_MCOLOR_WHITE, FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED);
