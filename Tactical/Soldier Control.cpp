...
//---legion by JAzz---------------------------------------
void SOLDIERTYPE::BeginSoldierOkno( void )
{
	INT8							ubDirection;

	if ( EnoughPoints( this, GetAPsToJumpThroughWindows( this, FALSE ), 0, TRUE ) )
	{
		if ( FindOknoDirection( this, this->sGridNo, this->ubDirection, &ubDirection ) )
		{
...
//---legion by JAzz---------------------------------------
void BeginSoldierOkno( SOLDIERTYPE *pSoldier )
{
	INT8							ubDirection;
	if ( EnoughPoints( pSoldier, GetAPsToJumpThroughWindows( pSoldier, FALSE ), 0, TRUE ) )
	{
	if ( FindOknoDirection( pSoldier, pSoldier->sGridNo, pSoldier->ubDirection, &ubDirection ) )
	{
		pSoldier->sTempNewGridNo = NewGridNo( pSoldier->sGridNo, (UINT16)DirectionInc(ubDirection ) );
		pSoldier->flags.fDontChargeTurningAPs = TRUE;
		EVENT_InternalSetSoldierDesiredDirection( pSoldier, ubDirection, FALSE, pSoldier->usAnimState );
