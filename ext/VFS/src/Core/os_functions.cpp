...
os::CIterateDirectory::CIterateDirectory(vfs::Path const& sPath, vfs::String const& searchPattern)
{
#ifdef WIN32
...
bool os::CIterateDirectory::nextFile(vfs::String &fileName, CIterateDirectory::EFileAttribute &attrib)
{
#ifdef WIN32
...
bool os::createRealDirectory(vfs::Path const& sDir, bool bDoNotCreate)
{
	...
	if(bDoNotCreate)
	{
		...
		if(!vfs::Settings::getUseUnicode())
		{
			...
			if(hFile == INVALID_HANDLE_VALUE)
			{
				...
				log << "Directory [" << sDir() << "] does not exist : " << error << vfs::Log::endl;
			}
		...
		else
		{
			...
			if(hFile == INVALID_HANDLE_VALUE)
			{
				DWORD error = GetLastError();
				vfs::Log log(L"errors.log", true);
				log << L"Directory [" << sDir() << L"] does not exist : " << error << vfs::Log::endl;
			}
	...
	if(success == 0)
	{
		...
		vfs::Log log(L"errors.log", true);
		log << L"Could not create directory [" << sDir() << L"] : " << error << vfs::Log::endl;
...
bool os::FileAttributes::getFileAttributes(vfs::Path const& sDir, vfs::UInt32& uiAttribs)
{
	...
	if(attribs == INVALID_FILE_ATTRIBUTES)
	{
		DWORD error = GetLastError();
		vfs::Log log(L"errors.log",true);
		log << L"Invalid File Attributes : " << error << vfs::Log::endl;
		return false;
	}
...
bool os::deleteRealFile(vfs::Path const& sDir)
{
	...
	if(!del)
	{
		...
		if(err != NO_ERROR)
		{
			VFS_ON_ERROR(BuildString(L"Could not delete file").get().c_str());
		}
	}
	return (del != FALSE);
#else
...
void os::getCurrentDirectory(vfs::Path& sPath)
{
	...
	{
		DWORD code = GetLastError();
		THROWEXCEPTION(BuildString().add(L"Could not determine current directory [").add(
			(!vfs::Settings::getUseUnicode()) ? L"[no unicode]" : L"[unicode]").add(L"],  error code : ").add(code).get());
	}
#else
...
bool os::getEnv(vfs::String const& key, vfs::String& value)
{
#ifdef _MSC_VER
