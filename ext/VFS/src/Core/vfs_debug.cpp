...
: std::exception() //(vfs::String::as_utf8(text).c_str())
{
	VFS_ON_ERROR(text);
	if(ex)
	{
	...
	IGNOREEXCEPTION(_time = vfs::String(datetime.substr(0,datetime.length()-1)));
...
CBasicException::CBasicException(vfs::String const& text, vfs::String const& function, int line, const char* file, CBasicException* ex)
: std::exception() //(text.utf8().c_str())
{
	VFS_ON_ERROR( text.c_str() );
	if(ex)
	{
		m_CallStack.insert(m_CallStack.end(), ex->m_CallStack.begin(), ex->m_CallStack.end());
	}
...
vfs::String CBasicException::getLastEntryString() const
{
...
vfs::String CBasicException::getExceptionString() const
{
	if(!m_CallStack.empty())
	{
		std::wstringstream wss;
